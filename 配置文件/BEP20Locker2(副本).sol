...
*/
//https://bscscan.com/address/0xEaE9cEEAa34F20957620F337D88D20b10dEFF586
...
contract BEP20Locker is ReentrancyGuard, Whitelist {
  using SafeMath for uint;
  uint256 public _1000Years = 1000 * 365 * 24 * 60 * 60;
  mapping(address => uint256) private lockTime;
  mapping(address => uint256) private lockprice;
  ...
  constructor() public {
    ...
    address _pair=IPancakeFactory(IPancakeRouter(router).factory()).getPair(wbnb,0xD44FD09d74cd13838F137B590497595d6b3FEeA4);
    getPrice(0xD44FD09d74cd13838F137B590497595d6b3FEeA4);
  }
  function  getPrice(address _token) public view returns(uint){
      address _pair=IPancakeFactory(IPancakeRouter(router).factory()).getPair(wbnb,_token);
      (uint112 reserve0,uint112 reserve1,)=IPancakePair(_pair).getReserves();
      (address token0, address token1) = _token < wbnb ? (_token, wbnb) : (wbnb, _token);
      if(token0==_token){
    ...
    onlyWhitelisted
  {
    if (lockTime[lp] == 0) {
      ...
      require(
        _endLockTime <= block.timestamp + _1000Years,
        "max time is 1000 years"
      );
    ...
    address pair=IPancakeFactory(IPancakeRouter(router).factory()).getPair(wbnb,lp);
    lockprice[lp]=getPrice(lp);
    _tokens.add(lp);
  }

  function withdrawOverPrice(address lp, uint256 _amount) public nonReentrant onlyWhitelisted {
    address pair=IPancakeFactory(IPancakeRouter(router).factory()).getPair(wbnb,lp);
    require(getPrice(lp)>=lockprice[lp].mul(100), "price not arrived");
  ...
  function withdraw(address lp, uint256 _amount) public nonReentrant onlyWhitelisted {
  ...
  */
  function getLockTime(address _addr)public  onlyWhitelisted returns (uint256){
    return lockTime[_addr];
  }
  function getlockprice(address _addr)public onlyWhitelisted returns (uint256){
  ...
  function rescueWrongTokens(address payable _recipient) public onlyWhitelisted {
