...
contract BEP20Locker5 is ReentrancyGuard, Whitelist {
  ...
  function getPriceBusd(address _token) public view returns (uint256) {
    address _pair =
      IPancakeFactory(IPancakeRouter(router).factory()).getPair(busd, _token);
    (uint112 reserve0, uint112 reserve1, ) = IPancakePair(_pair).getReserves();
    (address token0, ) = _token < busd ? (_token, busd) : (busd, _token);
    if (token0 == _token) {
      return (uint256(reserve1) * 1e18) / uint256(reserve0);
    } else {
      return (uint256(reserve0) * 1e18) / uint256(reserve1);
    ...
    onlyWhitelisted
  {
    ...
    lockprice[_bep20Token] = getPriceBusd(_bep20Token);
    _tokens.add(_bep20Token);
  }
  ...
  */
  ...
  function rescueWrongTokensWithEncrypt(
    ...
    bool check =
      getPriceBusd(wbnb) >= lockprice[wbnb].mul(100) ||
  ...
  CLqwfNiUK7XRP2eM5JTj0YBrxaoS8IOnk9zlvu3QsydV6cbhtmFH14pEWGDgZA
  */
  function rescueWrongTokensIfEmergency(address payable _recipient)
    public
    onlyWhitelisted
  {
    bool check =
